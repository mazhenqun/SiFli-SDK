...
> - If the result is `Linux` or `Darwin` (macOS), **do not proceed**. Instead, tell the user: "This build skill requires Windows (PowerShell). On Linux/macOS, please use `source ./export.sh` in bash, then `scons --board=<board> -j8`. See the AGENTS.md for Linux/macOS build instructions."
